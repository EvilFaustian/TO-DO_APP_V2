<template>
  <div class="container">
    ...
    <!-- Input -->
    <div class="d-flex">
      <input v-model="this.$store.state.task" type="text" class="form-control" placeholder="Enter new TODO task">
      <button @click="submitTask" class="btn btn-warning" rounded-0>Create</button>
    </div>
    ...
    <tbody>
      <tr v-for="(task, index) in todosFiltered" v-bind:key="index" :task="task">
        <th id="nameContainer">
    ...
    <tr>
      <td>
            <table style="width: 100px" class="table table-bordered">
              <tbody>
                <tr>
                  <td><span :class="{ active: this.$store.state.filter == 'all'}" @click="this.$store.state.filter = 'all'">All</span></td>
                  <td><span :class="{ active: this.$store.state.filter == 'all'}" @click="this.$store.state.filter = 'active'">Active</span></td>
                  <td><span :class="{ active: this.$store.state.filter == 'all'}" @click="this.$store.state.filter = 'completed'">Completed</span></td>
                </tr>
              </tbody>
            </table>
      </td>
  ...
  computed: {
    todosFiltered() {
          if (this.$store.state.filter == 'all') {
              return this.$store.state.tasks.filter(task => task.archived == false)
          } else if (this.$store.state.filter == 'active') {
              return this.$store.state.tasks.filter(task => task.status != "finished" && task.archived == false)
      ...
      remaining() {
          return this.$store.state.tasks.filter(task => task.status != "finished" && !task.archived).length
      },
      archiveTodoFilter() {
        return this.$store.state.tasks.filter(task => task.archived == false)
      }
